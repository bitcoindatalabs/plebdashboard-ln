...
function createSidebarManager() {
    ...
    function updateNodeInfo(nodeAttributes) {
        ...
        const pubKey = attrs.pubKey || nodeAttributes.pub_key || nodeAttributes.id || '';
        const nodeLabel = nodeAttributes.label || attrs.alias || pubKey || 'Node Details';
        const profileLink = pubKey ? `profile.html?node=${encodeURIComponent(pubKey)}` : '#';

        document.getElementById('node-info').innerHTML = `
            <div class="info-title">${nodeLabel}</div>
            <div class="info-content">
                ...
                <div><span class="info-label">Public Key:</span> ${pubKey ? `<a href="${profileLink}" style="color: var(--primary); text-decoration: underline; word-break: break-all;" title="View node profile">${attrs.pubKey || pubKey}</a>` : (attrs.pubKey || 'N/A')}</div>
